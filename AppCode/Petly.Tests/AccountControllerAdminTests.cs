...
public class AccountControllerAdminTests
{
    ...
    private static TestIdentityScope CreateIdentityScope(ApplicationDbContext db)
    {
        ...
        var serviceProvider = new TestServiceProvider(tokenProvider);
        ...
        var principalFactory = new UserClaimsPrincipalFactory<ApplicationUser, IdentityRole<int>>(userManager, roleManager, identityOptions);
        ...
        var accountService = new AccountService(db, userManager, roleManager);
        return new TestIdentityScope(db, userManager, roleManager, signInManager, accountService);
    }
    ...
    private sealed class TestServiceProvider(TestTokenProvider tokenProvider) : IServiceProvider
    {
        public object? GetService(Type serviceType)
        {
            return serviceType == typeof(TestTokenProvider) ? tokenProvider : null;
        }
    }
